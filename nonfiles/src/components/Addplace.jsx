...
async function Addplace(event) {
      event.preventDefault();
      await axios.post("http://localhost:4000/save-places", {
        owner: ownerId,
        title: title, 
        address: address,
        addPhotos: addPhotos,
  ...
  return (
    <div className=" grid grid-cols-2 bg-slate-100">
      ...
      <div className="cols-span-3 p-4 mx-auto items-center">
        <div className=" text-pink-400 text-3xl mb-5 font-bold mx-auto w-full justify-center">Traveligo</div>
        <div className="  rounded-2xl shadow-2xl justify-center flex flex-col  items-center  transition duration-1000 ease-in">
          ...
          <div className="flex flex-col items-center mt-2">
            ...
            <button type="submit" className="button m-2 w-1/3 rounded-full text-blue-600 bg-blue-400 bg-opacity-40 text-xl font-bold">Add Place</button>
          </div>
    ...
    <div className="grid grid-cols-2 mt-14">
     { addPhotos.length > 0 && addPhotos.map((photo)=>(
